...
theta = c(13.0890097,139.0550059,91.0700930,-1.3636700,0.3352523)

fit = optim(theta,lnLike,Ao=Ao,Ci=Ci)
fit
...
```{r}
newdata = data.frame(Ci = seq(min(Ci),max(Ci),length.out = 100))
```
...
for(i in 1:Nmc){
    ## sample inputs
    Tpred[i,] = theta.boot[sample.int(nrow(theta.boot), 1), ]
  
    Aconf[i,] = FvCB(Tpred[i,], newdata$Ci)
    Apred[i,] = rnorm(nrow(newdata),Aconf[i,],Tpred[i,5]) 
}
```
...
```{r}
CI = apply(Aconf,2,quantile, c(0.025,0.975))
PI = apply(Apred,2,quantile, c(0.025,0.975))

plot(Ci,Ao)
lines(newdata$Ci,FvCB(fit$par, Ci = newdata$Ci))

lines(newdata$Ci,CI[1,],col="blue")
lines(newdata$Ci,CI[2,],col="blue")
lines(newdata$Ci,PI[1,],col="green")
lines(newdata$Ci,PI[2,],col="green")

legend("bottomright", legend = c("Fit Line", "Confidence Intervals", "Prediction Intervals"),
       col = c("black", "blue", "green"), lty = c(1, 2, 3))
```

* Calculate a 95% interval for both the CI and PI
* Add these lines to the plot you made in Lab 10 Question 5 (data + best fit line)
...
```{r}
j = findInterval(400,newdata$Ci) ## Find the Ci value in newdata closest to 400 ppm
...
Vcmax and K are the most nonlinear. Both start out low and gradually increase. 

* Which parameter is photosynthetic rate the most sensitive to in absolute terms? Which the least?

Most sensitive: Vcmax
Least sensitive: r
...
Most impact: Vcmax
Not worth worrying about: definitely sigma, maybe r
...
Roughly 16% is not explained by the parameters. We are missing the interaction between terms and could take into account a covariance matrix.
