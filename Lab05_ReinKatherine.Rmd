...
for(time_spot in 1:length(Time)){
  lines(x, C_matrix[,time_spot], type = 'b', col = time_spot+1)
}
...
for(x_spot in 1:length(x)){
  lines(Time, C_matrix[x_spot,], type = 'b', col = x_spot+1)
}
...
# Set up new matrix
C_matrix2 = matrix(0,length(x),length(time_2)+1) # Add an additional column to handle exception
...
# Delete Last Column
C_matrix2 = C_matrix2[, -ncol(C_matrix2)]
...
for(t_spot in choose_time){
  lines(x, C_matrix2[,t_spot], type = 'b', col = colors_graph[n])
  n = n+1
}
...
colors_graph = c("red","orange","green","blue","purple")
n = 1
...
# Part A lines
lines(x, C_matrix[,2], type = 'b', col = colors_graph[1])
lines(x, C_matrix[,3], type = 'b', col = colors_graph[2])

# Part B lines
lines(x, C_matrix2[,11], type = 'b', col = colors_graph[3])
lines(x, C_matrix2[,101], type = 'b', col = colors_graph[4])
...
plot(Time, C_matrix[1,], main = "Concentration vs. Time (both)", ylab = "Concentration (g/cm)", xlab = "Time (s)", ylim = range(C_matrix), log = "x", type = 'b', col = colors_graph[1])

# Part A lines
lines(Time, C_matrix[2,], type = 'b', col = colors_graph[2])

# Part B lines
lines(time_2, C_matrix2[1,], type = 'l', col = colors_graph[3])
lines(time_2, C_matrix2[2,], type = 'l', col = colors_graph[4])
...
For concentration vs distance, the infinite series is quite a bit lower than the finite series. This makes sense as the concentration has less space to go in a finite series so the same amount of mass is in a smaller space causing the concentration to increase.

For concentration vs time, the infinite series is also quite a bit lower than the finite series. The main difference for this one, is that 0.5 finite starts out as the lowest, even lower than 0.5 infinite.
