...
```{r}
plot(x = NULL,y = NULL, ylab = "Concentration (g/cm)", xlab = "Time (s)", ylim = range(C_matrix[1,]), xlim = range(Time), log = 'x')
...
```{r}
plot(x = NULL,y = NULL, ylab = "Concentration (g/cm)", xlab = "Distance from Origin (cm)", ylim = range(C_matrix2[,1]), xlim = range(x))

for(time_spot in 1:length(Time)){
  lines(x, C_matrix2[,time_spot], type = 'l', col = time_spot+1)
}
...
```{r}
plot(x = NULL,y = NULL, ylab = "Concentration (g/cm)", xlab = "Time (s)", ylim = range(C_matrix2[1,]), xlim = range(time_2), log = 'x')

for(x_spot in 1:length(x)){
  lines(time_2, C_matrix2[x_spot,], type = 'l', col = x_spot+1)
}
